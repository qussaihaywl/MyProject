...
import { Search, Download, TrendingUp, Building2, DollarSign, Package, AlertCircle, BarChart3, PieChart as PieChartIcon } from "lucide-react";
import { toast } from "sonner";
import { trpc } from "@/lib/trpc";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line } from "recharts";

interface WarehouseCommission {
  id: number;
...
const COLORS = ["#f43f5e", "#B2291E", "#f97316", "#eab308", "#22c55e", "#06b6d4", "#3b82f6", "#8b5cf6"];
...
export default function WarehouseCommissionsDashboard() {
  ...
  const [sortBy, setSortBy] = useState<"date" | "amount" | "warehouse">("date");

  const commissions = commissionsQuery.data || [];

  const filteredAndSortedCommissions = useMemo(() => {
    let filtered = commissions.filter((c: WarehouseCommission) => {
      const matchesSearch = c.warehouseCode.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesStatus = filterStatus === "all" || c.status === filterStatus;
      return matchesSearch && matchesStatus;
    });

    filtered.sort((a: WarehouseCommission, b: WarehouseCommission) => {
      if (sortBy === "date") {
  ...
  const warehouseStats = useMemo(() => {
    ...
    (commissions as WarehouseCommission[]).forEach((c: WarehouseCommission) => {
      if (!grouped[c.warehouseCode]) {
        grouped[c.warehouseCode] = {
          name: c.warehouseCode,
          totalCommission: 0,
          totalOrders: 0,
          pending: 0,
          approved: 0,
      ...
      grouped[c.warehouseCode].totalCommission += parseFloat(c.commissionAmount);
      grouped[c.warehouseCode].totalOrders += 1;
      grouped[c.warehouseCode][c.status] += 1;
      grouped[c.warehouseCode].totalAmount += parseFloat(c.totalOrderAmount);
      if (c.orderId) {
        grouped[c.warehouseCode].orders.push(c.orderId);
      }
    });
  ...
  const stats = useMemo(() => {
    ...
    const pending = commissions.filter((c: WarehouseCommission) => c.status === "pending").length;
    const approved = commissions.filter((c: WarehouseCommission) => c.status === "approved").length;
    const paid = commissions.filter((c: WarehouseCommission) => c.status === "paid").length;
    const totalAmount = commissions.reduce((sum: number, c: WarehouseCommission) => sum + parseFloat(c.commissionAmount), 0);
    const totalWarehouses = new Set(commissions.map((c: WarehouseCommission) => c.warehouseCode)).size;
    const totalOrderAmount = commissions.reduce((sum: number, c: WarehouseCommission) => sum + parseFloat(c.totalOrderAmount), 0);

    return { total, pending, approved, paid, totalAmount, totalWarehouses, totalOrderAmount };
  }, [commissions]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-orange-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">إدارة وتتبع عمولات وحسابات المستودعات</p>
        </div>

        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-2 sm:p-2.5 md:p-3 lg:p-4 mb-8">
          ...
          <Card className="p-2.5 sm:p-3 md:p-2 sm:p-2.5 md:p-3 lg:p-4 lg:p-6 bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-purple-600 text-sm font-semibold">عدد المستودعات</p>
                <p className="text-sm sm:text-base md:text-lg sm:text-base sm:text-sm sm:text-base md:text-lg md:text-xl md:text-2xl font-bold text-purple-900">{stats.totalWarehouses}</p>
              </div>
              <Building2 className="w-10 h-10 text-purple-400 opacity-50" />
            </div>
          </Card>
          ...
          <Card className="p-2.5 sm:p-3 md:p-2 sm:p-2.5 md:p-3 lg:p-4 lg:p-6 bg-gradient-to-br from-indigo-50 to-indigo-100 border-indigo-200 shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-indigo-600 text-sm font-semibold">إجمالي المبلغ</p>
                <p className="text-sm sm:text-base md:text-lg sm:text-base sm:text-sm sm:text-base md:text-lg md:text-xl md:text-2xl font-bold text-indigo-900">{stats.totalAmount.toFixed(2)}</p>
              </div>
              <DollarSign className="w-10 h-10 text-indigo-400 opacity-50" />
            </div>
          </Card>
          ...
          <Card className="p-2.5 sm:p-3 md:p-2 sm:p-2.5 md:p-3 lg:p-4 lg:p-6 bg-gradient-to-br from-rose-50 to-rose-100 border-rose-200 shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-rose-600 text-sm font-semibold">إجمالي الطلبات</p>
                <p className="text-sm sm:text-base md:text-lg sm:text-base sm:text-sm sm:text-base md:text-lg md:text-xl md:text-2xl font-bold text-rose-900">{stats.totalOrderAmount.toFixed(0)}</p>
              </div>
              <Package className="w-10 h-10 text-rose-400 opacity-50" />
            </div>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:p-2 sm:p-2.5 md:p-3 lg:p-4 md:p-2.5 sm:p-3 md:p-2 sm:p-2.5 md:p-3 lg:p-4 lg:p-6 lg:p-8 mb-8">
          ...
          <Card className="p-2.5 sm:p-3 md:p-2 sm:p-2.5 md:p-3 lg:p-4 lg:p-6 shadow-lg border-rose-200">
            <h3 className="text-base sm:text-sm sm:text-base md:text-lg md:text-xl font-bold text-rose-900 mb-4">📊 عمولات المستودعات</h3>
            {warehouseStats.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={warehouseStats as any}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:p-2.5 md:p-3 lg:p-4">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="ابحث عن كود المستودع..."
            ...
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-gradient-to-r from-rose-50 to-pink-50 border-b-2 border-rose-200">
                  <TableRow>
                    <TableHead className="text-rose-900 font-bold">كود المستودع</TableHead>
                ...
                <TableBody>
                  {filteredAndSortedCommissions.map((commission: WarehouseCommission) => (
                    <TableRow key={commission.id} className="border-b border-rose-100 hover:bg-rose-50 transition-colors">
                      <TableCell className="font-bold text-rose-900">{commission.warehouseCode}</TableCell>
